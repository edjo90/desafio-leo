/* Funcoes Jquery implementada 2021 */
...
var modalregistros 	= document.getElementById('myModalRegistros');
var modalregistro   = document.getElementById('myRegistroModal');

// Obtenha o botão que abre o modal
var btn = document.getElementById('myBtnLogin');
var lnk = document.getElementById('myLnkRegistro');
var Ins = document.getElementById('myInsRegistro');
...
// o elemento <span> que fecha o modal Nivel 1
...
capturaEvento(window, 'onSubmit', function(evt){
	...
	// Localiza o link com id "formlogin"
...
function fn_listagem() {
    
	$.ajax({
			url		: "views/pesquisa_usuarios.php",
			type	: "post",
			dataType: "json",
			data 	: { param1: 1 } 
	}).done(function(r) {
			if (r.codzero==0) {
				$('#listagem').fadeOut(250).show()
					.html().fadeIn(300);
				modalregistro.style.display = 'none';
			} else {
				alert('Falha Inesperada! '+r.codzero);
			} 
	});
}
...
function in_usuario() {
	...
	var vlogin_mail 	= 	$('#login_mail').val();
	var vlogin_pwd 		= 	$('#login_pwd').val();
    var vlogin_nome		= 	$('#login_nome').val();
  
alert ('Cadastra Dados no Banco');
  
/*	
	$.ajax({
			url		: "views/insere_usuario.php",
			type	: "post",
			dataType: "json",
			data 	: { usuario_mail: vlogin_mail, usuario_pwd : vlogin_pwd, usuario_nome : vlogin_nome } 
	}).done(function(r) {
			if (r.codzero==0) {
				alert('Cadastrado '+r.codzero);
				modalregistro.style.display = 'none';
				$('#listagem').fadeOut(250).show()
					.html().fadeIn(300);

			} else {
				alert('Falha no Cadastro! '+r.codzero);
			} 
	});
*/	
	vlogin_mail = '';
